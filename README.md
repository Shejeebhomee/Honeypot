...
In this assignment, the goal was to set up a honeypot that could log information on any attacks, such as the attacker's IP address and the time the attack was performed. This honeypot was created by first cloning and running the Modern Honeypot Network (MHN), which provided a simple and streamlined method for setting up our low-interaction honeypot. Using vagrant and virtualbox, we set up two virtual machines, one for the server and the other for the honeypot itself. We then SSH'd into the server VM and installed the necessary MHN scripts. After doing so, we could access our MHN server in the host machine by navigating to http://10.254.254.100/, which gave us a nice GUI interface for monitoring how many attacks we recieve as well as a detailed logs of the attacks that were made. We then setup the Dionaea honeypot by SSH-ing into the honeypot VM and running the necessary configuration files. Finally, we could simulate an attack by running nmap against our honeypot address. The attacks page in the MHN server was then updated to reflect the nmap attack that was made.
